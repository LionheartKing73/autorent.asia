...
<?php
define( 'CHECKOUT_PAGE', '11') ;   // To dtermine which Extras are to be displayed
...
<?php
// Setup some input, request variables
...
// Load the One Way Costs

if ( $req_districtFr != $req_districtTo )
	$bc->LoadOWC($req_districtFr,$req_districtTo );

$price = $bc->CalcRegionalPrice($locationid,$req_districtFr,$req_districtTo, 0 ,0);
...
print "\n</form>";
	...
	if ( $_REQUEST["vehtype"] <> "Please Select" )
	{
        ...
        if ( $_REQUEST["vehtype"] != $_REQUEST["retloc"])
        {
             ...
             // Get the to location Id
             $SqlTo = "select locationid from locations where location_code = '";
             $SqlTo .= trim($_REQUEST["retloc"])."'";
      
    		 $ResultSetTo = mysql_query( $SqlTo ) ;
    		 $RecTo = mysql_fetch_array($ResultSetTo);
    		 $tolocationid = $RecTo["locationid"];
...
while($row = mysql_fetch_array($ResultSet)){
	...
	$schemeid = $row["pricing_scheme_id"]; 
	$luggage = $row["luggage"];
	$class = $row["class"];
    $air = $row[ "air" ];
	
	// if there is no Supplier price for a One Way Rental, ignore
	if (  $req_districtFr != $req_districtTo)
	{
		if ( ! $bc->OWCarray[ $supplierid ] )
			continue;
	}
    ...
    $Sql = "select * from stop_sell where fk_ct_locations_id = $locationid and " .
           ...
           " and fk_ct_pricing_scheme_id = $schemeid  " . 
    ...
    $Sql = "select * from closing_times where " .
   		 " fk_ct_supplier_id = $supplierid  " . 
    	 " and (( fk_ct_locations_id = $locationid and " .
    
           "closed_from <= '$StartTime' and closed_to > '$StartTime' ) " .
           " OR ( fk_ct_locations_id = $tolocationid and " .
	...
	$price = $bc->CalcRegionalPrice($locationid,$req_districtFr,$req_districtTo,$ID,$supplierid );
	...
	// note that the price could be 0 but the calculatiom would show a One Way fee
	if($bc->OnlinePrice - $bc->OWCprice != 0){  
    
        if ( $class )
            $class_array[$class] = $class;  
        $htmltext = "\n<div id='Vehicle".$ID."' class='Vehicle Vehicle".$class."'>";  
		$htmltext .= "\n<form method='post' action='register_booking.php'>";
...
//		$htmltext .= '<input type="input" name="Submit" value="'.$ID.'" src="img/b_booking.gif" width="118" height="21" border="0">';
		...
		$htmltext .= "\n<td class='vehfield' colspan='6'>".$luggage."</td>";
...
{
	// Split the output onto 2 rows at the first occurrence of MPV
	if (  $ck == "MPV6"  )
	{
