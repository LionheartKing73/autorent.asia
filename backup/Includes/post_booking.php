...
if ( $vehtype != $retloc )
    ...
    $bc->CalcPrice( $StDateArr[2],$StDateArr[1],$StDateArr[0],$StTimeArr[0],$StTimeArr[1],$EnDateArr[2],$EnDateArr[1],$EnDateArr[0],$EnTimeArr[0],$EnTimeArr[1], $ID );


    $sql = "insert into bookings ";

    $sql .= "( fk_booking_currencies_id, name, address, address2,";
    ...
    $sql .= "( $SessCurrencyId, '".$PostName."', '".$PostAddress."', '".$PostAddress2."', ";
    ...
    $sql .= "'".$SessCalcPrice."', '".$SessBookingBalance."','".$PostRemark."', ".$PartnerVisitId." ) ";
...

    {
        ...
        $body .= "\nPickup: ".$vehtype." on ".$PickupDate." at ".$PickupTime;

        $body .= "\nDrop Off: ".$retloc." on ".$DropoffDate." at ".$DropoffTime;
        $body .= "\nDays: ".$bc->NumDays;
        $body .= "\nFlight: ".$PostFlightNo;
        ...
        $body .= "\nOptional Waiver Insurance: ".$EmailOptWaiverText; 
        if ( $bc->OptWaiver  )
            $body .= "\nOptional Waiver Insurance Cost: ".number_format( $bc->Insurance,2 ); 
        $body .= "\nOptional Accident Insurance: ".$EmailOptAccidentText; 
        if ( $bc->OptAccident  )
            $body .= "\nOptional Accident Insurance Cost: ".number_format( $bc->OptAccidentCost,2 );   
        $body .= "\nChild Seats: ".$bc->ChildSeats;   
        if ( $bc->ChildSeats  )
        {
            $ChildSeatsCost = $bc->ChildSeats *$bc->OptChildSeatCost;
            $body .= "\nOptional Child Seats Cost: ".number_format( $ChildSeatsCost,2 );   
        }  
        $body .= "\nOptional Thai GPS: ".$EmailOptGpsThText;  
        if ( $bc->OptGpsTh  )
            $body .= "\nOptional Thai GPS Cost: ".number_format( $bc->OptGpsThCost,2 );   
        $body .= "\nOptional English GPS: ".$EmailOptGpsEnText; 
        if ( $bc->OptGpsEn  )
            $body .= "\nOptional English GPS Cost: ".number_format( $bc->OptGpsEnCost,2 );       
        
        $body .= "\nTotal Extras: ".number_format( $bc->Extras,2); 
        ...
        $body .= "\nPrice Quoted: ".number_format($SessCalcPrice,2);

        $upfront =   $SessCalcPrice - $SessBookingBalance;

        $body .= "\nDeposit: ".number_format($upfront,2);  

        $body .= "\nBalance To Pay: ".number_format($SessBookingBalance,2);
