...
$PostCardEmail = substr( $_REQUEST["cc_email"], 0, 30 );          
    ...
    $EnTimeArr = explode( ":", $DropoffTime);  
            ...
            // No calc no of days
    $bc = new BookingCosts( $CountryRec["country_code"], 1, $StDateArr[2],$StDateArr[1],$StDateArr[0],$StTimeArr[0],$StTimeArr[1],$EnDateArr[2],$EnDateArr[1],$EnDateArr[0],$EnTimeArr[0],$EnTimeArr[1] );
...
if ( $vehtype != $retloc )
    ...
    $vrow = mysql_fetch_array($ResultSet);
     ...
     $sql = "select supplierid FROM vehicles WHERE vehicleid = '".$ID."' ";
    $ResultSet = mysql_query( $sql ) ;

    $vehrow = mysql_fetch_array($ResultSet);
    ...
    $price = $bc->CalcRegionalPrice( $vrow["locationid"],$req_districtFr,$req_districtTo, $ID, $Row["supplierid"] );
    print "<!--DOug";
    var_dump( $bc );
    print "-->";
...

    {
        ...
        IF ( ! $stop )
        {
             $sql = "SELECT price, extras_text, checkout_page FROM booking_extras be " .
