...
<h2 class="Banner"><b>CHANGE YOUR CHOICE</b></h1>
        <div id="menu">
...
?>
 ...
 // DISTRICTS
        ...
        $Sql =  "select district.*, country_subdomain from district " . 
                "JOIN country on country_id = fk_district_country_id WHERE country_id =1 order by district_order, district_id";
 ...
 // LOCATIONS
        ...
        $Sql =  "select locations.*, district.*, country_subdomain from locations " . 
                ...
                "JOIN country on country_id = fk_district_country_id WHERE country_id =1 order by district_order,district_id, location_order, location_name";
        ...
        while ($row = mysql_fetch_array($ResultSet) ) {
            ...
            if ( $_REQUEST[ "locFr".$row["country_subdomain"]."-".$row["district_id"]  ] ==  $row["location_code"])
                $selected = "selected";
            else
                $selected = "";
            $locFr[$row["country_subdomain"]][$row["district_id"]].="\n<option $selected value='".$row["location_code"]."'>";
            $locFr[$row["country_subdomain"]][$row["district_id"]].=$row["location_name"]."</option>";
            
            if ( $_REQUEST["locTo".$row["country_subdomain"]."-".$row["district_id"]] ==  $row["location_code"])
                $selected = "selected";
            else
                $selected = "";          
        ...
        foreach( $locFr as $key=>$arr )
        {
                ...
                foreach( $arr as $dist=>$loctext )
                {
                  ...
                  elseif ( $first_iteration)
                  {
                    ...
                    // for the first district (unless it has already been passed in )
                    if ( ! $_REQUEST["vehtype"] )
                        $display="XYXY-sub"; 
                    else
                         $display="none";
                    ...
                    $display="none";
                  $ctryFr[$key] .= "<select style='width: 220px; display: ".$display."' class='locFr' id='locFr".$key."-".$dist."' name='locFr".$key."-".$dist."'>"; 
                  $ctryFr[$key] .= $loctext;
                  $ctryFr[$key] .= "</select>";
                }
                if ( $displaying_locs  )
        ...
        foreach( $locTo as $key=>$arr )
        {
                ...
                foreach( $arr as $dist=>$loctext )
                {
                  ...
                  elseif ( $first_iteration)
                  {
                    ...
                    $display="none";
                  $ctryTo[$key] .= "<select style='width: 220px; display: ".$display."' class='locTo' id='locTo".$key."-".$dist."' name='locTo".$key."-".$dist."'>"; 
                  $ctryTo[$key] .= $loctext;
                  $ctryTo[$key] .= "</select>";
                }
                if ( $displaying_locs || $_REQUEST["retloc"])
...
<input type="hidden" name="distformFlag" value='Go'/>
